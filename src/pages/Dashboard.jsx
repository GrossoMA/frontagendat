...
const useObtainEventos = () => {
  ...
  const { data, isPending, error } = useFetch({ endpoint: endpointAllEventos, requestOptions })
    ...
    <>
      <Menu>
        ...
        <div className='flex flex-col gap-10 justify-center'>
          ...
          <div>
            <hr />
            {isPendingEventos ? <Loader /> : <ControlEventos data={dataEventos} />}
          </div>
        </div>
      </Menu>
    </>
...
const ControlEventos = ({ data }) => {
  ...
  return (
    <>
      <h2 className='text-blue-500 text-xl font-bold text-center'>Eventos Cargados</h2>
      <label htmlFor='min-stock' className='font-bold text-blue-500 flex justify-center items-center gap-4 mb-2'>Proximo Evento:
        {/* <input
          onChange={() => handleMinStock(event)}
          min={0}
          id='min-stock'
      ...
      <table className='w-full p-2'>
        ...
        {data?.messaje
          ...
          {data?.map(d => {
              ...
              return (
               <tr key={d.id_evento} className='border-b'>
                 <td className='text-center'>{d.nombre_evento}</td>
                 <td className='text-center'>{d.id_localidad}</td>
                 <td className='text-center'>{d.fecha_inicio}</td>
               </tr>
              )
